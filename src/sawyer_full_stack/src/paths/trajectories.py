...
class LinearTrajectory(Trajectory):
    def __init__(self, start_position, goal_position, desired_orientation, total_time):
    ...
    def target_velocity(self, time):
        ...
        6x' :obj:`numpy.ndarray`
            desired body-frame velocity of the end effector
        """
        pose = self.target_pose(time)
        g = utils.get_g_matrix(pose[:3], pose[3:])
        body_vel = np.ones(4)
        if time <= self.total_time / 2.0:
            ...
            body_vel[:3] = self.acceleration * time
        else:
            ...
            body_vel[:3] = self.acceleration * (self.total_time - time)
        
        homo_vel = np.linalg.inv(g) @ body_vel
        linear_vel = homo_vel[:3]
...
class CircularTrajectory(Trajectory):
    ...
    def target_velocity(self, time):
        ...
        vel_d = np.ndarray.flatten(self.radius * theta_dot * np.array([-np.sin(theta), np.cos(theta), 0, 1]))
        vel_d = np.linalg.inv(g) @ vel_d
